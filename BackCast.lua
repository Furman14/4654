local BackCast = {}
BackCast.IsEnabled = Menu.AddOptionBool({ "Utility","BackCast" }, "Enabled", "")
...
function BackCast.OnUpdate(orders)
	if myHero and used == 1 and not NPC.IsRunning(myHero) then
...
function BackCast.OnPrepareUnitOrders(orders)
	if orders.order ~= 5 then 
		return true
	end
	if not Menu.IsEnabled(BackCast.IsEnabled) then
		return true
	end
	...
	if (Ability.GetCastRange(orders.ability) + NPC.GetCastRangeBonus(myHero) < (orders.position - Entity.GetAbsOrigin(myHero)):Length2D()) and (not (abil_name == "tinker_march_of_the_machines")) then
		return true
	end	
